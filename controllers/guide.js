...
export const register = async (req, res, next) => {
    const { firstName, lastName, contactNo, gender, age, location, email, password } = req.body;
    ...
    const guide = await Guide.create({
        firstName,lastName, contactNo, gender, age, location, email,
...
export const fetchDashboardInfo = async (req, res, next) => {
    try {
        const guide = await Guide.findById({ id: req.user.id }, { password: false })
